# -*- coding: utf-8 -*-
"""
Created on Fri Oct 11 18:59:39 2019
...
# 동영상 편집
"""
VIDEO_FILE_PATH = 'K:\\data\\ssgvideotest\\ssgproject.avi'
...
while(cap.isOpened()):
    ...
    if int(cap.get(1)) % 2 == 0:
        ...
        cv2.imwrite("K:\\data\\ssgvideotest\\ssgproject_frame\\frame%d.jpg" % cap_count, image)
...
for frame_i in range(947, 1057):

    path="K:\\data\\ssgvideotest\\final_frame\\frame%d.jpg" %frame_i

    img, img2 = transform_img(path)
    ...
    with tf.Session() as sess:
        ...
        sess.run(init)
        saver.restore(sess, 'C:\\data\\ssg_Test1\\model9_name\\vgg8_128_model')
        ...
        idx = 0
        for (x,y,w,h) in faces:
            if pic_list[idx][1] > 0.98:
                img = cv2.rectangle(img, (x - int(w / 5),y - int(h / 5) ), (x + w + int(w / 5),y + h + int(h / 5) ), col[pic_list[idx][0]], 3)
                img = cv2.putText(img, namelabel[pic_list[idx][0]], (x - int(w / 5),y - int(h / 5) -5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, col[pic_list[idx][0]], 1)
                idx += 1
            else:
                img = cv2.rectangle(img, (x - int(w / 5),y - int(h / 5) ), (x + w + int(w / 5),y + h + int(h / 5) ), (225,225,225), 3)
            
        
        cv2.imwrite("K:\\data\\ssgvideotest\\final_face_frame\\frame%d.jpg" %frame_i, img)
...
for frame_i in range(0, 1057):
    ...
    cv2.imwrite("K:\\data\\ssgvideotest\\only_white_sq\\frame%d.jpg" %frame_i, img)
